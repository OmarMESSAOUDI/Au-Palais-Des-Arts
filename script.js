...
function afficherErreur(champ, message) {
    ...
    erreurElement.style.color = '#e74c3c';
    erreurElement.style.fontSize = '0.9rem';
    erreurElement.style.marginTop = '0.3rem';
...
// ===== GESTION DES AVIS =====
document.querySelector('.btn-voir-plus-avis')?.addEventListener('click', function() {
    afficherNotification('Chargement des avis supplémentaires...');
    // Ici vous pourriez charger plus d'avis via AJAX
});
...
document.getElementById('btnCommander')?.addEventListener('click', function() {
    ...
    afficherNotification('Redirection vers la page de commande...');
    // Ici vous redirigeriez vers la page de commande
});
...
// S'assurer que les fonctions ne sont pas redéfinies
if (typeof window.panierInitialise === 'undefined') {
    window.panierInitialise = true;
}
